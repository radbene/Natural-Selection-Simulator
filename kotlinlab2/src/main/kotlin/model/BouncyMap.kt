...
class BouncyMap(private val width: Int, private val height: Int) : IWorldMap {
    ...
    private fun isInBounds(position: Vector2d): Boolean = position.precedes(Vector2d(width - 1, height - 1)) && position.follows(Vector2d(0, 0))

    override fun canMoveTo(position: Vector2d): Boolean =
       isInBounds(position) && !isOccupied(position)
    ...
    override fun place(animal: Animal) {
        ...
        if (isOccupied(animal.position)) {
            val newPosition: Vector2d? = animals.randomFreePosition(Vector2d(width, height))
            if (newPosition != null)
                animal.position = newPosition
            else
                throw RuntimeException("Map is null or full")
        }
